    ...
    <header className={cn(
      ...
      <nav className="section-container" aria-label="Main navigation">
        <div className={cn(
          ...
          <Link
            to="/"
            className="group flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-primary/50 rounded-xl"
            aria-label="Qatar Internet Exchange - Home"
          >
            <div className={cn(
              "relative flex items-center justify-center rounded-xl font-bold text-lg overflow-hidden transition-all duration-500",
              "bg-gradient-to-br from-primary via-primary to-primary-dark text-white",
              scrolled ? "h-10 w-10" : "h-12 w-12",
              "shadow-lg group-hover:shadow-glow group-hover:scale-105"
            )}>
            ...
            <div className={cn(
              ...
              <div className={cn(
                ...
                scrolled ? "text-base text-foreground" : "text-lg text-white"
              )}>
            ...
            {navItems.map((item) => (
              ...
              >
                ...
                <span className={cn(
                  ...
                  isActive(item.href) 
                    ? "w-4 bg-secondary" 
                    : "w-0 bg-transparent group-hover:w-2 group-hover:bg-secondary/50"
                )} />
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={cn(
                "relative flex items-center gap-2 h-10 px-4 rounded-xl font-semibold text-sm transition-all duration-500 overflow-hidden group",
                "focus:outline-none focus:ring-2 focus:ring-primary/50",
                scrolled 
            ...
            <button
              type="button"
              className={cn(
                "lg:hidden flex items-center justify-center h-10 w-10 rounded-xl transition-all duration-300",
                "focus:outline-none focus:ring-2 focus:ring-primary/50",
                scrolled 
            ...
            >
              <div className="relative w-5 h-5">
                <span className={cn(
                  "absolute left-0 block h-0.5 w-5 bg-current rounded transition-all duration-300",
                  mobileMenuOpen ? "top-2.5 rotate-45" : "top-1"
                )} />
                <span className={cn(
                  "absolute left-0 top-2.5 block h-0.5 bg-current rounded transition-all duration-300",
                  mobileMenuOpen ? "w-0 opacity-0" : "w-3 opacity-100"
                )} />
                <span className={cn(
                  "absolute left-0 block h-0.5 w-5 bg-current rounded transition-all duration-300",
      ...
      >
        ...
        {/* Panel */}
        <div className={cn(
          "absolute top-0 right-0 h-full w-full max-w-sm bg-background shadow-2xl transition-transform duration-500 ease-out",
          mobileMenuOpen ? "translate-x-0" : "translate-x-full",
          isRTL && "right-auto left-0",
          isRTL && (mobileMenuOpen ? "translate-x-0" : "-translate-x-full")
        )}>
          {/* Panel Header */}
          ...
          <div className="p-6 space-y-2 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 180px)' }}>
            {navItems.map((item, index) => (
              <Link
                ...
                to={item.href}
                className={cn(
                  "flex items-center justify-between p-4 rounded-xl text-base font-medium transition-all duration-300",
                  "focus:outline-none focus:ring-2 focus:ring-primary/50",
                  isActive(item.href)
